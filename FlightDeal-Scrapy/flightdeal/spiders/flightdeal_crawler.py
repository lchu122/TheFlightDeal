...
class FlightDealCrawlerSpider(CrawlSpider):
    ...
    allowed_domains = ["theflightdeal.com"] # best not to include http://
    ...
    def parse_page2(self, response):
        ...
        while pos < len(body) and not found:
            if re.search('(V|v)alid for travel', self.u2s(body[pos])) or re.search('(W|w)e found availability', self.u2s(body[pos])) is not None:
                ...
                item['travel_dates'] = self.date_extractor(self.u2s(posting_date), travel_date_string)
                found = True
            else:
    ...
    def date_extractor(self, post_date, date_string):
        ...
        # from January 2017 - December 2017 or August 2018
        ...
        or_without_to = re.findall('(?<=or )((early |mid |late )?(January|February|March|April|May|June|July|August|September|October|November|December)( \d+(th|st|nd|rd))?(, \d{4})?)(?! -)', \
        date_string)
        ...
        # this should be general order oh how dates appear, but no guarantee. May have to sort later.
        # or just if until, but written for clarity.
        if until != []:
            parsed_dates += '/' + post_date + ' - ' + '/'.join([g[0] for g in until])
